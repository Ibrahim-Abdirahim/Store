...
public class CartService {
    ...
    // Get the current user's cart — creates one if they don't have one yet
    ...
    // Add a product to the cart — if it's already there, just increase the quantity
    @Transactional
    ...
    // Load a user from the DB by email
    private User getUser(String email) {
        return userRepository.findByEmail(email)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));
    }

    // Convert Cart entity to CartResponse DTO including total price calculation
    private CartResponse toResponse(Cart cart) {
        ...
        // Add up all item subtotals to get the cart total
